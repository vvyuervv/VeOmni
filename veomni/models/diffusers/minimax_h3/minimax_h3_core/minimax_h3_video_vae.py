...
class BaseConv3d(nn.Conv3d):
    def __init__(
        ...
        self.pad_mode_t = ("constant" if padding_mode_t == "zeros" else padding_mode_t) or (
    ...
    def forward(self, x):
        ...
        if self.padding[2] or self.padding[1]:
            x = F.pad(
                x, (self.padding[2], self.padding[2], self.padding[1], self.padding[1], 0, 0), mode=self.pad_mode
            )
...
class RotaryEmbeddingND(nn.Module):
    ...
    def forward(self, img_ids):
        with torch.autocast(get_device_type(), enabled=False):
    ...
    def __init__(
        ...
        for i in range(num_levels):
            ...
            down.block = nn.ModuleList(
                [
                    ResnetBlock3D(
                        in_channels=block_in[i] if j == 0 else block_mid[i],
                        out_channels=block_mid[i],
            ...
            if space_down[i] * time_down[i] > 1:
                down.downsample = Downsample3D(
                    block_mid[i], block_mid[i], time_stride=time_down[i], space_stride=space_down[i], **conv_kwargs
                )
            self.down.append(down)
        self.norm_out = _group_norm(block_mid[-1], use_t_isolated_gn)
        self.conv_out = BaseConv3d(
            block_mid[-1], 2 * z_channels if double_z else z_channels, kernel_size=3, padding=1, **conv_kwargs
        )
...
def _pack_tensors_3d(x, ps, ps_t):
    B, C, T, H, W = x.shape
    return (
        x.view(B, C, T // ps_t, ps_t, H // ps, ps, W // ps, ps)
        .permute(0, 2, 4, 6, 1, 3, 5, 7)
        .reshape(B, (T // ps_t) * (H // ps) * (W // ps), C * ps_t * ps * ps)
    )
...
def _unpack_tensors_3d(x, ps, ps_t, T, H, W):
    ...
    c = C // (ps_t * ps * ps)
    return (
        x.view(B, T // ps_t, H // ps, W // ps, c, ps_t, ps, ps)
...
class ViT3DDecoder(nn.Module):
    ...
    def forward(self, x):
        ...
        return _unpack_tensors_3d(
            output, self.patch_size, self.patch_size_t, T * self.patch_size_t, H * self.patch_size, W * self.patch_size
        )
...
class VAEProcessor:
    ...
    def revert_tensor(self, x):
        if x.ndim == 5:
            B, _, T, _, _ = x.shape
            x = rearrange(
                self.transform_rev(rearrange(x, "b c t h w -> (b t) c h w")).clamp(0, 1),
                "(b t) c h w -> b c t h w",
                b=B,
                t=T,
            )
...
class MiniMaxH3VideoVAE(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.processor = VAEProcessor(
            Normalize(mean, std), Normalize(tuple(-m / s for m, s in zip(mean, std)), tuple(1.0 / s for s in std))
        )
    ...
    def tiled_encode(self, x):
        ...
        for i, row in enumerate(rows):
            ...
            for j, tile in enumerate(row):
                if i > 0:
                    tile = self.blend(rows[i - 1][j], tile, ly[i - 1], dim=-2)
                if j > 0:
                    tile = self.blend(row[j - 1], tile, lx[j - 1], dim=-1)
                if i < i_max - 1:
                    tile = tile[..., : -ly[i], :]
                if j < j_max - 1:
                    tile = tile[..., :, : -lx[j]]
    ...
    def tiled_decode(self, z):
        ...
        tiles = [
            z[
                ...,
                y_idx[i] // self.vae_ratio : (y_idx[i] + y_len[i]) // self.vae_ratio,
        ...
        for i, row in enumerate(rows):
            ...
            for j, tile in enumerate(row):
                if i > 0:
                    tile = self.blend(rows[i - 1][j], tile, y_ovlp[i - 1], dim=-2)
                if j > 0:
                    tile = self.blend(row[j - 1], tile, x_ovlp[j - 1], dim=-1)
                if i < i_max - 1:
                    tile = tile[..., : -y_ovlp[i], :]
                if j < j_max - 1:
                    tile = tile[..., :, : -x_ovlp[j]]
    ...
    def encode_temporal(self, x):
        ...
        z = torch.cat(
            [
                self.tiled_encode(x[:, :, i * self.clip_length : (i + 1) * self.clip_length])
    ...
    def _decode_temporal_pad_frames(self, z, pad_tokens):
        ...
        return sum(
            intra_tail if (z_len + k) % self.tokens_chunk_size == 0 else self.vae_ratio_t for k in range(pad_tokens)
        )
    ...
    def _decode_temporal_streaming(self, z, num_chunks, pad_tokens):
        ...
        def write_part(part):
            ...
            if dec is None:
                dec = torch.empty(
                    *part.shape[:2], output_frames, *part.shape[3:], dtype=part.dtype, device=part.device
                )
        ...
        for i in range(num_chunks):
            ...
            for j in range(split_count):
                f_start = j * chunk_dec
                chunk = clip_dec[:, :, f_start : min(f_start + chunk_dec, clip_dec.shape[2])][
        ...
        if logical_frames != total_frames or dropped_frames != pad_frames or write_pos != output_frames:
            raise RuntimeError(
                f"decode_temporal frame plan mismatch: logical={logical_frames} total={total_frames} dropped={dropped_frames} pad={pad_frames} pos={write_pos} out={output_frames}"
            )
        return dec
